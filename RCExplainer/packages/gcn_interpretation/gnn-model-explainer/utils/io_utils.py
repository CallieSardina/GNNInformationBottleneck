...
def load_ckpt(args, isbest=False):
    ...
    if os.path.isfile(filename):
        ...
        ckpt = torch.load(filename, weights_only=False) # HERE -- added 'weights_only = False'
    else:
...
def log_graph(
    ...
    node_size_def = 50
    if args is not None:
        if args.bmname is not None and args.bmname == 'synthetic':
            feat_labels = {}
            use_labels = True
            node_names = ['A','B','C','D','E','F','G','H','I','J','K','L']
            for i in Gc.nodes():
                feat_labels[i] = node_names[np.argmax(Gc.nodes[i]['feat'])]
            font_size_def = 12
            node_size_def = 150
    ...
    if max(edge_colors) == min(edge_colors):
    ...
    #
    # print("node colors:", len(node_colors), node_colors[0])
    #
    print("edges:", Gc.number_of_edges())
    #
    # print("edge colors:", len(edge_colors), edge_colors[0])
...
def read_graphfile(datadir, dataname, max_nodes=None, edge_labels=False):
    ...
    for i in range(1, 1 + len(adj_list)):
        ...
        gt[i-1] = []
        Ggt = nx.relabel_nodes(G, mapping)
        for c, edge in enumerate(Ggt.edges):
            if edge_label_list[i][2 * c] > 0:
                gt[i-1].append((edge, edge_label_list[i][2 * c]))
            assert(edge_label_list[i][2 * c] == edge_label_list[i][2 * c + 1])
        if i == 1:
            print(Ggt.edges)
            print(gt[i-1])
        # indexed from 0
        graphs.append(nx.relabel_nodes(G, mapping))
    torch.save(gt, 'Mutagenicity_gt_edge_labels_new.p')
    return graphs

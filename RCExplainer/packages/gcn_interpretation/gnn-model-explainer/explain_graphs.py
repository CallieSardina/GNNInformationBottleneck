...
def get_mutagenicity_model(input_dim, num_classes, device, args):
    return models.GcnEncoderGraph(
        ...
        num_layers=3,
        pred_hidden_dims=[256, 128, 64, 32], #HERE
        bn=False,
        args=args,
        device=device
    )
...
def train_explainer(explainer, model, rule_dict, adjs, feats, labels, preds, num_nodes, args, graph_indices):
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        for graph_idx in graph_indices:
            ...
            emb, embs_non_cat = model.getEmbeddings(x, adj, batch_num_nodes=[sub_nodes.cpu().numpy()])
            emb = emb.clone().detach()

            # print("len emb in train explainer: ", len(emb), len (embs_non_cat))
            if emb.shape[0] > 1:
                all_embs.append(emb)

            for i, layer in enumerate(embs_non_cat):
                if i not in embs_dict:
                    embs_dict[i] = [] 
                embs_dict[i].append(layer.cpu().detach()) 
        ...
        true_Y_np = labels.cpu().detach().numpy().reshape(-1, 1)   # Ensure correct shape


        print("Y shape: ", true_Y_np.shape)
        print("Unique values in true_Y_np:", np.unique(true_Y_np, return_counts=True))
        ...
        for i, layer_embs in embs_dict.items():
            ...
            print(f"Layer {i} emb min: {layer_embs_np.min()}, max: {layer_embs_np.max()}, mean: {layer_embs_np.mean()}")


            
            layer_0_embs_np = np.array(embs_dict[0]).mean(axis=1) 

            if layer_embs_np.shape == (1, 1) or true_Y_np.shape == (1, 1) or layer_0_embs_np.shape == (1, 1):
                continue

            mi_XZ = [EDGE(layer_0_embs_np, layer_embs_np)]
            mi_ZY = [EDGE(layer_embs_np, true_Y_np)]

            # print("Y: ", true_Y_np)
            # print("Z: ", layer_embs_np)
            # print("mi_ZY: ", mi_ZY)
        ...
        log_filename = f'./MI_logs/mi_log_layers_fc.txt'
    ...
    return explainer, all_embs, embs_dict
...
class ExplainModule(nn.Module):
    ...
    def loss(self, pred, pred_label, node_idx=None, graph_embedding=None, boundary_list=None, gt_embedding=None, inv_embedding=None):
        ...
        # HERE -- commenting out these losses and adding fc layers
        # # size
        # mask = self.mask
        ...
        loss = net_boundary_loss # + size_loss + mask_ent_loss # HERE

...
class GSAT(nn.Module):
    ...
    def train_one_batch(self, data, epoch):
        ...
        # compute MI
        g = graph_emb.clone().detach().numpy()
        g_s = graph_emb_sub.clone().detach().numpy()
        y = data.y.clone().detach().numpy()
    ...
    #     att, loss, loss_dict, clf_logits, embeddings_per_layer = self.forward_pass(data, epoch, training=True)

    #     # Extract subgraph embeddings for each layer
    #     # subgraph_emb = self.extract_subgraph(data, epoch, tag='train')

    #     # Loop over subgraphs and compute MI for each subgraph embedding
    #     for subgraph in subgraph_emb:
    #         g_s = subgraph.detach().numpy()  # Subgraph embeddings
    #         g = embeddings_per_layer[0].detach().numpy()  # Graph embeddings
    #         y = data.y.detach().numpy()  # Graph labels

    #         # Compute Mutual Information (MI) using EDGE for each subgraph
    #         mi_xz = EDGE(g_s, g)  # MI between subgraph embeddings and graph embeddings
    #         mi_zy = EDGE(g_s, y)  # MI between subgraph embeddings and graph labels

    #         # Append MI values to the lists
    #         mi_XZ.append(mi_xz)
    #         mi_ZY.append(mi_zy)

    #     # Average MI over all subgraphs
    #     avg_mi_XZ = np.mean(mi_XZ)
    #     avg_mi_ZY = np.mean(mi_ZY)

    #     # Log average MI values
    #     with open('./run_results_gsat.txt', 'a') as f:
    #         print(f"Epoch {epoch}, Avg MI_XZ: {avg_mi_XZ}, Avg MI_ZY: {avg_mi_ZY}", file=f)

    #     # # Extract subgraph embeddings for each layer
    #     # subgraph_emb = self.extract_subgraph(data, epoch, use_edge_attr)

    #     # # Compute Mutual Information (MI) using EDGE
    #     # g = embeddings_per_layer[0].detach().numpy()  # g_s: subgraph embeddings
    #     # g_s = subgraph_emb.detach().numpy()  # g: graph embeddings
    #     # y = data.y.detach().numpy()  # y: graph labels

    #     # # MI calculations
    #     # mi_XZ = EDGE(g_s, g)  # MI between g_s and g
    #     # mi_ZY = EDGE(g_s, y)  # MI between g_s and y

    #     # # # print("Emb per layer shape: ", len(embeddings_per_layer))
    #     # # mi_XZ = EDGE(embeddings_per_layer[0].detach().numpy(), embeddings_per_layer[0].detach().numpy())
    #     # # mi_ZY = EDGE(data.y.detach().numpy(), embeddings_per_layer[0].detach().numpy())
    #     # with open('./run_results_gsat.txt', 'a') as f:
    #     #     print(f"Epoch {epoch}, MI_XZ: {mi_XZ}, MI_ZY: {mi_ZY}", file=f)
...
class ExtractorMLP(nn.Module):
    ...
    def __init__(self, hidden_size, shared_config):
        ...
        fc_hidden_dim = hidden_size // 2  
        self.fc1 = nn.Linear(hidden_size, fc_hidden_dim)
        self.fc2 = nn.Linear(fc_hidden_dim, hidden_size)
...
def main():
    ...
    print('====================================')
    print('====================================')
    print(f'[INFO] Running {method_name} on {dataset_name} with {model_name} -- fc layers in extractor MLP')
    print('====================================')
